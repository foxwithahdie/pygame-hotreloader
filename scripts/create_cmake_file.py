...
from parse_cmake.parsing import File, Arg, Command, _Command, BlankLine

from scripts.include_lib_paths import PYTHON_GLOBAL_INCLUDE, PYGAME_INCLUDE, \
                                      PYGAME_LIB, PYGAME_SECOND_LIB, SDL2_INCLUDE, \
                                      SDL2_LIB, SDL2_BIN, INSTALLED_GLOBALLY, localize_path, \
                                      convert_path_sep
...
def configure_cmake_file(cmake_config_file: File) -> None:
    ...
    config_file_body: list[_Command] = [
        ...
        Command(name='project',
                body=[
                    Arg(contents="pygame-hotreloader")
                ]
        ), BlankLine(),
        ...
        Command(name="target_include_directories",
                body=[
                    ...
                    Arg(contents="PRIVATE"),
                    Arg(contents=f"{convert_path_sep(PYTHON_GLOBAL_INCLUDE) \
                                    if "win32" in sys.platform else PYTHON_GLOBAL_INCLUDE}"),
                    Arg(contents="PRIVATE")
                ] + ([
                    Arg(contents=f"{SDL2_INCLUDE}")
                ] if INSTALLED_GLOBALLY else [
                    Arg(contents=f"{cmake_source_dir}/{localize_path(SDL2_INCLUDE)}")
                ])
        ), BlankLine(),
        Command(name="target_link_directories",
                body=[
                    Arg(contents=project_name_ref),
                    Arg(contents="PRIVATE"),
                ...
                ] if not INSTALLED_GLOBALLY else []) + ([
                    Arg(contents="PRIVATE"),
                    Arg(contents=f"{cmake_source_dir}/{localize_path(PYGAME_SECOND_LIB)}")
    ...
    if INSTALLED_GLOBALLY:
        config_file_body.insert(len(config_file_body) - 5, Command(
            name="find_package",
            body=[
                Arg(contents="SDL"),
